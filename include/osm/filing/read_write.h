...
#ifdef _MSC_VER
  ium::disable_invalid_parameter_handler diph;
#endif
...
/**
 * Writes the given number of bytes from the output_buffer to the file
 * descriptor. This is just a wrapper around write(2), because write(2) can
 * write less than the given number of bytes.
 *
 * @param fd File descriptor.
 * @param output_buffer Buffer with data to be written. Must be at least size
 * bytes long.
 * @param size Number of bytes to write.
 * @throws std::system_error On error.
 */
inline void reliable_write(const int fd,
  ...
  do {
    ...
    int64_t length = 0;
    do {
      length = ::write(fd, output_buffer + offset,
                       static_cast<unsigned int>(write_count));
...
inline void reliable_write(const int fd,
                           ...
                           const size_t size) {
  reliable_write(fd, reinterpret_cast<const unsigned char*>(output_buffer),
                 size);
}
 ...
 *
 * @param fd File descriptor.
 * @param input_buffer Buffer for data to be read. Must be at least size
 bytes
 * long.
 ...
 * @throws std::system_error On error.
 */
inline int64_t reliable_read(const int fd,
                             char* input_buffer,
 ...
 * this function will continue reading until either EOF or an error is
 * encountered.
 *
...
inline bool read_exactly(int fd, char* buffer, unsigned int size) {
  ...
  while (to_read > 0) {
    auto const read_size =
        reliable_read(fd, buffer + (size - to_read), to_read);
    if (read_size == 0) {  // EOF
      return false;
    }
