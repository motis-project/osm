...
enum { mmap_vector_size_increment = 1024UL * 1024UL };
...
protected:
  std::size_t m_size = 0;
  ium::TypedMemoryMapping<T> m_mapping;

public:
  mmap_vector_base(const int fd,
                   const std::size_t capacity,
                   const std::size_t size = 0)
      : m_size(size),
        m_mapping(
            capacity, ium::MemoryMapping::mapping_mode::write_shared, fd) {
    assert(size <= capacity);
    std::fill(data() + size, data() + capacity, ium::empty_value<T>());
    shrink_to_fit();
  }

  explicit mmap_vector_base(
      const std::size_t capacity = mmap_vector_size_increment)
      : m_mapping(capacity) {
    std::fill_n(data(), capacity, ium::empty_value<T>());
  }
  ...
  void close() { m_mapping.unmap(); }

  std::size_t capacity() const noexcept { return m_mapping.size(); }
  ...
  void shrink_to_fit() {
    while (m_size > 0 && data()[m_size - 1] == ium::empty_value<value_type>()) {
      --m_size;
    }
  }
  ...
  void reserve(const std::size_t new_capacity) {
    if (new_capacity > capacity()) {
      ...
      std::fill(data() + old_capacity, data() + new_capacity,
                ium::empty_value<value_type>());
    }
  }
