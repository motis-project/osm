...
struct OSMIUM_EXPORT not_found : public std::runtime_error {
  ...
  explicit not_found(const std::string& what) : std::runtime_error(what) {}

  explicit not_found(const char* what) : std::runtime_error(what) {}

  explicit not_found(uint64_t id)
      : std::runtime_error(std::string{"id "} + std::to_string(id) +
                           " not found") {}
