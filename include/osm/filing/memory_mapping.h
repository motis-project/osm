 ...
 * mapping.unmap();                      // release mapping by calling unmap()
 * (or at end of scope)
 * @endcode
 *
 ...
 * int fd = ::open(...);
 * {
 *     MemoryMapping mapping{1024, MemoryMapping::mapping_mode::write_shared,
 * fd, offset};
 *     // use mapping
 * }
 * ::close(fd);
...
private:
  ...
  int resize_fd(int fd) const {
    ...
    auto const current_file_size = file_size(fd);
    if (current_file_size < m_size + m_offset) {
      ...
      resize_file(fd, m_size + m_offset);
    }
    return fd;
  }
...
public:
   ...
   * @throws std::system_error if the remapping fails
   */
  void resize(std::size_t new_size) { m_mapping.resize(sizeof(T) * new_size); }

  /**
   * In a boolean context a TypedMemoryMapping is true when it is
   * a valid existing mapping.
   */
  explicit operator bool() const noexcept { return !!m_mapping; }
   ...
   * @returns file descriptor, -1 for anonymous mappings
   */
  int fd() const noexcept { return m_mapping.fd(); }

  /**
   * Was this mapping created as a writable mapping?
   */
  bool writable() const noexcept { return m_mapping.writable(); }
   ...
   * @pre is_valid()
   */
  T* begin() noexcept { return m_mapping.get_addr<T>(); }
   ...
   * @pre is_valid()
   */
  T* end() noexcept { return m_mapping.get_addr<T>() + size(); }
   ...
   * @pre is_valid()
   */
  const T* cbegin() const noexcept { return m_mapping.get_addr<T>(); }
   ...
   * @pre is_valid()
   */
  const T* cend() const noexcept { return m_mapping.get_addr<T>() + size(); }
   ...
   * @pre is_valid()
   */
  const T* begin() const noexcept { return m_mapping.get_addr<T>(); }
   ...
   * @pre is_valid()
   */
  const T* end() const noexcept { return m_mapping.get_addr<T>() + size(); }
...
inline bool ium::MemoryMapping::is_valid() const noexcept {
  return m_addr !=
         MAP_FAILED;  // NOLINT(cppcoreguidelines-pro-type-cstyle-cast,performance-no-int-to-ptr)
}

inline void ium::MemoryMapping::make_invalid() noexcept {
  m_addr =
      MAP_FAILED;  // NOLINT(cppcoreguidelines-pro-type-cstyle-cast,performance-no-int-to-ptr)
}
...
inline int ium::MemoryMapping::get_protection() const noexcept {
...
inline int ium::MemoryMapping::get_flags() const noexcept {
  if (m_fd == -1) {
...
inline ium::MemoryMapping::MemoryMapping(std::size_t size,
                                         mapping_mode mode,
                                         int fd,
                                         off_t offset)
    : m_size(check_size(size)),
      ...
      m_mapping_mode(mode),
      m_addr(::mmap(
          nullptr, m_size, get_protection(), get_flags(), m_fd, m_offset)) {
...
inline ium::MemoryMapping::MemoryMapping(MemoryMapping&& other) noexcept
    : m_size(other.m_size),
...
inline ium::MemoryMapping& ium::MemoryMapping::operator=(
...
inline void ium::MemoryMapping::unmap() {
  if (is_valid()) {
...
inline void ium::MemoryMapping::resize(std::size_t new_size) {
  ...
  if (m_fd == -1) {  // anonymous mapping
    ...
    m_size = new_size;
    resize_fd(m_fd);
    m_addr = ::mmap(nullptr, new_size, get_protection(), get_flags(), m_fd,
                    m_offset);
    if (!is_valid()) {
...
/* References:
 * CreateFileMapping:
 * https://msdn.microsoft.com/en-us/library/aa366537(VS.85).aspx CloseHandle:
 * https://msdn.microsoft.com/en-us/library/ms724211(VS.85).aspx MapViewOfFile:
 * https://msdn.microsoft.com/en-us/library/aa366761(VS.85).aspx
 * UnmapViewOfFile:
 * https://msdn.microsoft.com/en-us/library/aa366882(VS.85).aspx
 */

namespace ium {

inline DWORD dword_hi(uint64_t x) { return static_cast<DWORD>(x >> 32); }

inline DWORD dword_lo(uint64_t x) { return static_cast<DWORD>(x & 0xffffffff); }

// namespace util

}  // namespace ium

inline DWORD ium::MemoryMapping::get_protection() const noexcept {
  switch (m_mapping_mode) {
    case mapping_mode::readonly: return PAGE_READONLY;
    case mapping_mode::write_private: return PAGE_WRITECOPY;
...
inline DWORD ium::MemoryMapping::get_flags() const noexcept {
  switch (m_mapping_mode) {
    case mapping_mode::readonly: return FILE_MAP_READ;
    case mapping_mode::write_private: return FILE_MAP_COPY;
    default:  // mapping_mode::write_shared
      break;
  }
...
inline HANDLE ium::MemoryMapping::get_handle() const noexcept {
  if (m_fd == -1) {
...
inline HANDLE ium::MemoryMapping::create_file_mapping() const noexcept {
  if (m_fd != -1) {
    _setmode(m_fd, _O_BINARY);
  }
  return CreateFileMapping(
      get_handle(), nullptr, get_protection(),
      osmium::dword_hi(static_cast<uint64_t>(m_size) + m_offset),
      osmium::dword_lo(static_cast<uint64_t>(m_size) + m_offset), nullptr);
}

inline void* ium::MemoryMapping::map_view_of_file() const noexcept {
  return MapViewOfFile(m_handle, get_flags(), osmium::dword_hi(m_offset),
                       osmium::dword_lo(m_offset), m_size);
}
...
inline void ium::MemoryMapping::make_invalid() noexcept { m_addr = nullptr; }
...
inline ium::MemoryMapping& ium::MemoryMapping::operator=(
    ium::MemoryMapping&& other) noexcept {
  try {
    unmap();
  } catch (const std::system_error&) {  // NOLINT(bugprone-empty-catch)
    // Ignore unmap error. It should never happen anyway and we can't do
