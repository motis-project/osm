...
#define WIN32_LEAN_AND_MEAN  // Prevent winsock.h inclusion; avoid winsock2.h
                             // conflict
#endif
#include <crtdbg.h>
#include <io.h>
...
inline std::size_t file_size(const char* name) {
  ...
  if (::_stati64(name, &s) != 0) {
    throw std::system_error{
        ...
        std::string{"Could not get file size of file '"} + name + "'"};
  }
#else
  // Unix implementation
  struct stat s;  // NOLINT clang-tidy
...
inline void resize_file(int fd, std::size_t new_size) {
  ...
  ium::disable_invalid_parameter_handler diph;
  assert(new_size <=
         static_cast<std::size_t>(std::numeric_limits<__int64>::max()));
