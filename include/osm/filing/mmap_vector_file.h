...
class mmap_vector_file : public mmap_vector_base<T> {
  ...
  static std::size_t filesize(const int fd) {
    const auto size = ium::file_size(fd);
...
public:
  mmap_vector_file()
      : mmap_vector_base<T>(ium::create_tmp_file(),
                            ium::mmap_vector_size_increment) {}

  explicit mmap_vector_file(const int fd)
      : mmap_vector_base<T>(
            fd,
            std::max(static_cast<std::size_t>(mmap_vector_size_increment),
                     filesize(fd)),
            filesize(fd)) {}
